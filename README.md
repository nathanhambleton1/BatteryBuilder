...
A 2.5 h simulation of 195 detailed cells takes **~95 s**.
...
fixed-step discrete solver at `Ts`. The Pulse Generator is therefore **sample based**:
...
|---|---|
| 1 `iCell` | per-cell current, A (195) |
| 2 `numCyclesCell` | per-cell discharge cycles (195) |
| 3 `socCell` | **per-cell SOC (195) — feeds `Goto socCells`** |
| 4 `socParallelAssembly` | per-assembly SOC (195) |
| 5 `vCell` | **per-cell terminal voltage, V (195) — feeds `Goto cellV`** |
| 6 `vParallelAssembly` | per-assembly voltage, V (195) |

plus the `+` and `-` electrical terminals. That is what replaced the Simscape **Probe**
block the model used to need: tap ports 3 and 5 and you are done. The four unused ports
are terminated.

### The three fixups `P45B build` applies
...
the throwaway model. Three mask settings are re-applied with the copy:
...
error if `config()` disagrees, so you can never silently simulate the wrong pack.
...
- Mean SOC tops out at **0.947 instead of 0.998**, and pack voltage at **802 V instead of
  821 V**. The imbalance directly costs usable capacity. This is exactly why packs need
  balancing.
- The end-of-run spread is about **190 mV / 9.5 SOC points**.
...
- **Runtime** is ~95 s for the default 2.5 h run. The pack block probes six per-cell
  vectors instead of the old Probe's two, which costs about 15 s. Shorten `stopTime` in
  `config()` if you want faster iteration; ~1.4 h still reaches the first rest.
...
- **The four unused pack ports** (`iCell`, `numCyclesCell`, `socParallelAssembly`,
  `vParallelAssembly`) are terminated in the model. Delete a Terminator and wire the port
  up if you want the signal — no rebuild needed.
